...
const SCAN_SOURCES = [
  ...
  // Note: reeperbahn-festival.de has TLS issues — using HTTP fallback for scanner
...
const RADIO_SEED_DATA = [
  ...
  ['RAD019','Spotify Editorial',     'International', 'All genres',                 'https://artists.spotify.com',        'https://artists.spotify.com/help/article/pitch-a-song','Pitch via Spotify for Artists ≥7 days pre-release.',                            '','','High'],
];
...
function runWeeklyScan() {
  const result = scanSources();
  Logger.log('Weekly scan: ' + JSON.stringify(result));
}
